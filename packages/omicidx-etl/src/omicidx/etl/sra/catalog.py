...
from ..log import LogProgress, get_logger, log_operation
...
class SRACatalog:
    ...
    def get_completed_entities(self, mirror_entries: list[SRAMirrorEntry]) -> set[str]:
        ...
        for entry in mirror_entries:
            if (
                entry.in_current_batch
                and entry.is_full
                and self._done_marker_path(entry).exists()
            ):
                completed.add(entry.entity)
        return completed

    def _current_batch_dirs(
        self, mirror_entries: list[SRAMirrorEntry], entity: str
    ) -> set[str]:
    ...
    def cleanup_entity_by_filesystem(
        ...
        for date_dir in date_dirs:
            ...
            try:
                stage_dirs = [
                    p for p in date_dir.iterdir() if p.name.startswith("stage=")
                ]
            except (FileNotFoundError, OSError):
                continue
    ...
    def cleanup(
        self,
        mirror_entries: list[SRAMirrorEntry],
        completed_entities: set[str] | None = None,
    ) -> None:
        """
