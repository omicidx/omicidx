class BaseType:
    ...
    def __repr__(self):
        return f"<{self.__type__} class name:{self.name}>"
...
class ObjectType(BaseType):
    ...
    def mapping(self):
        return {"properties": {k: self.fields[k].mapping() for k in self.fields}}
...
class NestedType(BaseType):
    ...
    def mapping(self):
        return {
            ...
            "type": "nested",
            "properties": {k: self.fields[k].mapping() for k in self.fields},
        }
...
def walk_schema(schema, parent=None):
    ...
    for field in schema["fields"]:
        if field["type"] == "string":
            parent.fields[field["name"]] = StringType(
                name=field["name"], nullable=field["nullable"]
            )
        if field["type"] == "integer":
            parent.fields[field["name"]] = IntType(
                name=field["name"], nullable=field["nullable"]
            )
        if field["type"] == "long":
            parent.fields[field["name"]] = LongType(
                name=field["name"], nullable=field["nullable"]
            )
        if field["type"] == "timestamp":
            parent.fields[field["name"]] = DateType(
                name=field["name"], nullable=field["nullable"]
            )
        if field["type"] == "boolean":
            parent.fields[field["name"]] = BooleanType(
                name=field["name"], nullable=field["nullable"]
            )
        if isinstance(field["type"], dict):
            ...
            if n_field["type"] == "array":
                ...
                parent.fields[field["name"]] = walk_schema(
                    n_field["elementType"], parent=newp
                )
    return parent
